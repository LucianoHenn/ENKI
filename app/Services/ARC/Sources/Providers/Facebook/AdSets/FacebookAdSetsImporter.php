...
use App\Models\ARC\Providers\Facebook\FacebookAdSetsReportData;
...
/**
 * Class FacebookAdSetsImporter
 */
class FacebookAdSetsImporter extends BaseImporter
{
    ...
    public function doImport(ReportLogbook $request)
    {
        ...
        Log::info("[FacebookAdSetsImporter] Start importing for identifier {$identifier}");
        ...
        try {
            ...
            if (empty($json)) { // Note: depends from source
                Log::info("[FacebookAdSetsImporter] Empty data on {$localReport}");
                return false;
            }
    ...
    private function processRecord($row, ReportLogbook $request, ClientArcAssociation $activeAssoc)
    {
        ...
        $bid_amount_eur = CurrencyConversion::convertAmount($request->date_end, $account_currency, 'EUR', $bid_amount, 4, true);
        $bid_amount_usd = CurrencyConversion::convertAmount($request->date_end, $account_currency, 'USD', $bid_amount, 4, true);
